...
/*
        ...
        public void downloadSaveFile(URL url) throws IOException {
            ...
            try {
				...
				// Get the input stream length
                int fileLength = input.available();
                publishProgress(DownloadCallback.Progress.GET_INPUT_STREAM_SUCCESS, 0);
                if (input != null) {
					String filename = "/sdcard/android_robot.png";
                    output = new FileOutputStream(filename);
					// fill the missing part for saving the downloaded file
                    Bitmap bitmap = BitmapFactory.decodeStream(input);
                    bitmap.compress(Bitmap.CompressFormat.PNG, 100, output);
                    output.flush();
                    Log.d("File",String.valueOf(fileLength)+":finish");
                }
